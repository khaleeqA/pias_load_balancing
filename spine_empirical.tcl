...
set FailureCase [lindex $argv 38]

## flowbender ttl -- 2021 Oct 29
set flowBender [lindex $argv 39]

## Khaliq 2021-12-01 for FlowBender enablement

set mAckPkts_F [lindex $argv 40]
set totalAckPkts [lindex $argv 41]
set flowBender_T [lindex $argv 42]
set flowBenderon [lindex $argv 43]
### result file
...
puts "FailureCase $FailureCase "
puts "FlowBender $flowBender"
...
#Agent/TCP/FullTcp set checkCongestionFB 0
Agent/TCP/FullTcp set totalAckPkts 0
Agent/TCP/FullTcp set flowBender_T 0
Agent/TCP/FullTcp set flowBenderon 0
...
if {[string compare $switchAlg "Priority"] == 0 } {
    ...
    #Agent/TCP/FullTcp set checkCongestionFB $checkCongestionFB
    Agent/TCP/FullTcp set markedAckPkts_F $mAckPkts_F
    Agent/TCP/FullTcp set totalAckPkts $totalAckPkts
    Agent/TCP/FullTcp set flowBender_T $flowBender_T
    Agent/TCP/FullTcp set flowBenderon $flowBenderon
}
...
if {$enableMultiPath == 1} {
	...
	Classifier/MultiPath set flowBender_ $flowBender
...
#if {$FailureCase==1} {
   #puts "hello"
#$ns bandwidth $s(1) $n(3) 0.1Mb 'duplex'
#   $ns duplex-link $n(0) $a(0) [expr [set link_rate]/10]Gb 10ms DropTail
#}
 #$ns bandwidth $ $s2 $n6  0.1Mb 'duplex'
...
for {set i 0} {$i < $topology_tors} {incr i} {
    for {set j 0} {$j < $topology_spines} {incr j} {
         if { $FailureCase==1 && $i==0 && $j==0 } {
      		$ns duplex-link $n($i) $a($j) [expr $UCap/10]Gb $mean_link_delay $switchAlg
       	 } else {
...
#### 2021-10-13 Attempt to create partial failure on the fly
#if {$FailureCase==1} {
	###$ns bandwidth $s(1) $n(3) 0.1Mb 'duplex'
   	###$ns duplex-link $s(1) $n(3) 10Mb 10ms DropTail
	###ns at [expr FailureStartTime+FailureDuration] "$ns bandwidth $s(1) $n(3) [expr $bw_torAgg]Mb 'duplex'";
	#ns at 1.001 "$ns bandwidth $n(0) $a(0) 1Gb 'duplex'"; ## Commented out on 5pm, 2021-10-15
#}



#foreach {key value} [array get $topology_tors] {
#    puts "$key => $value"
#}

#if {$FailureCase==1} {
   #puts "hello"
 #    $ns bandwidth $ $S2 $n1  0.1Mb 'duplex'
 #   $ns duplex-link $a1 $n4 10Mb 10ms DropTail
#}
