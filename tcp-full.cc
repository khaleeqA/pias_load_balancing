...
FullTcpAgent::delay_bind_dispatch(const char *varName, const char *localName, TclObject *tracer)
{
    ...
    if (delay_bind(varName, localName, "flowBender_T", &flowBender_T, tracer)) return TCL_OK; // khaliq 
    ...
    if (delay_bind(varName, localName, "flowBender_", &flowBender_, tracer)) return TCL_OK; // khaliq 
    if (delay_bind(varName, localName, "flowbenderopen", &flowbenderopen, tracer)) return TCL_OK; // khaliq
...
if (ecnbit) {
	...
	if(debug_dctcp) {
		fprintf(stdout,"DCTCP update_alpha 1: (ackno=%i), (dctcp_alpha_update=%f),  (acked_bytes=%d)\n", (int)ackno, (double)dctcp_alpha_update_seq, acked_bytes);
