...
FullTcpAgent::delay_bind_init_all()
{
	...
	/* delay_bind_init_one("checkCongestionFB");  // khaliq */
	delay_bind_init_one("markedAckPkts_F"); // khaliq
	delay_bind_init_one("totalAckPkts");  //khaliq
	delay_bind_init_one("flowBender_T");  //khaliq  perflowMP
        delay_bind_init_one("flowBenderon");  //khaliq
        delay_bind_init_one("perflowMP");        
        delay_bind_init_one("numRTTsCongested_");   //khaliq
        delay_bind_init_one("flowBender_N");  //khaliq
...
FullTcpAgent::delay_bind_dispatch(const char *varName, const char *localName, TclObject *tracer)
{
    ...
    /*if (delay_bind(varName, localName, "checkCongestionFB", &checkCongestionFB, tracer)) return TCL_OK; // khaliq */
    if (delay_bind(varName, localName, "markedAckPkts_F",&markedAckPkts_F, tracer)) return TCL_OK; // khaliq
    if (delay_bind(varName, localName, "totalAckPkts", &totalAckPkts, tracer)) return TCL_OK; // khaliq
    if (delay_bind(varName, localName, "flowBender_T", &flowBender_T, tracer)) return TCL_OK; // khaliq perflowMP
    if (delay_bind(varName, localName, "flowBenderon", &flowBenderon, tracer)) return TCL_OK; // khaliq 
    if (delay_bind(varName, localName, "perflowMP", &perflowMP, tracer)) return TCL_OK; // khaliq     
    if (delay_bind(varName, localName, "numRTTsCongested_", &numRTTsCongested_, tracer)) return TCL_OK; // khaliq
    if (delay_bind(varName, localName, "flowBender_N", &flowBender_N, tracer)) return TCL_OK; // khaliq 
...
/*void SackFullTcpAgent:checkCongestionFB()   
 {
*/
///khaliq
int
...
#endif
		recover_ = maxseq_;
                ///scb_->ClearScoreBoard(); ///make error
	}else {
		if(tno == TCP_FB_TIMER) {
			/* FlowBender Timer 
			if(debug_dctcp) {
...
//khaliq
void FullTcpAgent::checkCongestionFB() {
	printf("My name is Khaliq!\n");
	/*double now = Scheduler::instance().clock();
      if(debug_dctcp) 
                {printf("this is FB portion");//khaliq
		printf("checkCongestionFB -> Time now: %f markedAckPkts_F=%f totalAckPkts=%f flowBender_T=%f \n", now, markedAckPkts_F, totalAckPkts, flowBender_T);
	        }
...
if((totalAckPkts > 0) && (fractionMarked > flowBender_T)) {
		if(++numRTTsCongested_ >= flowBender_N) {
			...
			if(debug_dctcp) {
                                  printf("this is FB portion");//khaliq
				printf("checkCongestionFB -> Rerouting Time now: %f markedAckPkts_F=%f totalAckPkts=%f defttl_=%d numRTTsCongested_=%d ", now, markedAckPkts_F, totalAckPkts, defttl_, numRTTsCongested_);
				printf("ndatapack_=%d \n", (int) ndatapack_);
			}
			numRTTsCongested_ = 0;
		}
...
//Shuang: prob count when cwnd=1
 if(debug_dctcp) 
                {
                 printf("khaliq 1.99");
		// printf("checkCongestionFB -> Time now: %f flowBender_T=%f \n", now,  flowBender_T);
	        }
	if (prob_cap_ != 0) {
		prob_count_ ++;
...
double 
FullTcpAgent::rtt_timeout() {


 if(flowBenderon == 1) 
                {
                 printf("khaliq 1.79999");
                 // double now = Scheduler::instance().clock();
	         printf(" flowBender_T=%f \n", flowBender_T);
                 printf(" totalAckPkts=%f \n", totalAckPkts);
                 printf(" markedAckPkts_F=%f \n", markedAckPkts_F);
               
                  // printf("this portion is working");
	        }
