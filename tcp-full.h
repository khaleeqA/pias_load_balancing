...
public:
	FullTcpAgent() :
		...
		informpacer(0), enable_pias_(0), pias_prio_num_(0), pias_debug_(0),
		 markedAckPkts_F(0), totalAckPkts(0), flowBender_T(0),numRTTsCongested_(0), 
                 flowBender_N(0), dctcp_total(0), dctcp_marked(0), next_pkt_(0), firstpartial_(0),
	         dctcp_alpha_update_seq(0), ce_transition(0),flowBender_(0),flowbenderopen(0) { }
                 // khaliq
...
protected:
	...
	// FlowBender variables -- Khaliq
         // void checkCongestionFB();
        virtual void checkCongestionFB();
        virtual void update_dctcp_alpha(Packet *pkt);
	// int flowBender_;
        int markedAckPkts_F; 
	int totalAckPkts; 
	double flowBender_T;
...
// khaliq 04-01-22 5:00 am
        int dctcp_total;  // khaliq
       int dctcp_marked; ///khaliq 
	double dctcp_alpha_update_seq;
//	int dctcp_maxseq;
	int ce_transition;
...
// khaliq 04-01-22 5:00 am
	...
	*/
	...
	virtual int send_allowed(int);
        virtual double rtt_timeout();		// ok to send this seq#?
