...
public:
	FullTcpAgent() :
		...
		informpacer(0), enable_pias_(0), pias_prio_num_(0), pias_debug_(0),
		 markedAckPkts_F(0), totalAckPkts(0), flowBender_T(0),numRTTsCongested_(0), flowBender_N(0), flowBenderon(0),
                 perflowMP(0),dctcp_total(0), dctcp_marked(0), dctcp_alpha_update_seq(0), ce_transition(0), partial_ack_(0) { }
		// Mohammad: added informpacer
		//Wei: add enable_pias_

// dctcp_alpha_(0), temp_alpha_(0),dctcp_g_(0)
...
protected:
	...
	virtual int byterm();
         virtual double  rtt_timeout();
           ...
           void update_dctcp_alpha(Packet *);
	/* Wei: PIAS priority */
